...
(state / 'status.json').write_text(json.dumps({'status':'Automatisch bijwerken uitgeschakeld door gebruiker','updated_at':datetime.datetime.now().isoformat(timespec='seconds'),'device':'iPhone'}, indent=2))
