    ...
    var body: some View {
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 26) {
                ...
                if let progress = trakt.progress(for: item) {
                    Toggle("Verder kijken vanaf \(Int(progress))%", isOn: $resumePlayback)
                        .font(.title3)
                }
                    ...
                    ScrollView(.vertical) {
                        LazyVStack(alignment: .leading, spacing: 18) {
                            ForEach(sources) { source in
                                NavigationLink {
                                    PlayerView(source: source, item: item, resumeProgress: resumePlayback ? trakt.progress(for: item) : nil)
                                } label: {
                                    sourceCard(source)
                                }
                                .buttonStyle(.card)
